...
3. Face Grouping/Clustering:
   ...
   - Threshold: ~0.5-0.6 for same person (TODO: make this an input param to tune based on needs)
...
Model Files Used:
   - /data/models/ultraface-version-RFB-320.onnx (or see ultraface.onnx)
   - /data/models/arcface_w600k_r50.onnx (or see arcface.onnx)
...
public interface IFaceDetectionProcessor 
{
    string GetOriginalFilePath(FileData dbPath);
     AlbumImage? GetAlbumImage(FileData dbPath);    
}
...
public class FaceDetectionService 
{
    ...
    public FaceDetectionService(PicturesDataConfiguration configuration, DatabaseConfiguration dbConfig)        
    {
    ...
    public async Task<int> ProcessFile(FileData dbPath, IFaceDetectionProcessor processor, bool logIfCreated = false)
    {
        if (!_modelsAvailable) return 0;
        if (_configuration.IsMovieFile(dbPath.FilePath)) return 0;

        var file = processor.GetOriginalFilePath(dbPath);
        if (!File.Exists(file)) return 0;

        // Get album_image from FileData (DbPeriodicScanService passes AlbumImage as Data)
        var albumImage = processor.GetAlbumImage(dbPath);
        if (albumImage == null)
        {
